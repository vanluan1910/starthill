  ...
  return (
    <section className="relative h-full w-full overflow-hidden bg-[#021816] text-white">
      ...
      <div className="relative z-10 mx-auto flex h-full min-h-full w-full max-w-[1280px] flex-col overflow-x-hidden overflow-y-auto overscroll-y-contain touch-pan-y px-4 pb-28 pt-16 sm:px-6 sm:pt-20 md:grid md:grid-cols-[minmax(0,0.95fr)_minmax(360px,0.75fr)] md:items-center md:gap-12 md:px-16 md:pb-20 md:pt-24 lg:gap-20">
        ...
        >
          <motion.div className="mb-3 sm:mb-5 flex items-center justify-center gap-3 sm:gap-4 md:justify-start" variants={itemVariants}>
            <span className="h-px w-8 sm:w-12 bg-[#d6a252]" />
            <span className="text-[10px] sm:text-[11px] font-extrabold uppercase tracking-[0.2em] sm:tracking-[0.24em] text-[#f0c975]">
              {t('welcome.heroLabel', lang)}
            </span>
          </motion.div>

          <motion.h1
            className="font-serif text-[28px] font-bold leading-[1.15] tracking-[-0.03em] text-white text-shadow xs:text-[34px] sm:text-[48px] md:max-w-[720px] md:text-[66px] lg:text-[80px]"
            variants={itemVariants}
          >
            {lang === 'ko' ? (
              <>
                <span className="block text-[#f0c975]">Star Hill Resort</span>
                <span className="block text-white/90 text-[22px] xs:text-[26px] sm:text-[40px] md:text-[52px] lg:text-[64px]">에 오신 것을 환영합니다</span>
              </>
            ) : (
              <>
          ...
          <motion.p
            className="mx-auto mt-3 sm:mt-5 max-w-[520px] font-serif text-[18px] sm:text-[22px] italic leading-tight text-white/90 md:mx-0 md:text-[28px]"
            variants={itemVariants}
          >
            {t('welcome.guest', lang)}
          </motion.p>

          <motion.div className="mx-auto mt-4 sm:mt-6 max-w-[620px] space-y-3 sm:space-y-4 md:mx-0" variants={itemVariants}>
            <p className="text-[14px] sm:text-[16px] leading-[1.65] sm:leading-[1.75] text-white/[0.85] md:text-[18px]">
              {t('welcome.para1', lang)}
            </p>
            <p className="border-l-2 border-[#d6a252]/[0.55] pl-4 sm:pl-5 text-left text-[13px] sm:text-[14px] italic leading-[1.6] sm:leading-[1.7] text-white/[0.7] md:text-[16px]">
              {t('welcome.para2', lang)}
            </p>
          </motion.div>

          <motion.div className="mt-6 sm:mt-8 flex flex-col justify-center gap-2.5 sm:gap-3 sm:flex-row md:justify-start" variants={itemVariants}>
            <button
              className="group inline-flex min-h-[46px] sm:min-h-[52px] items-center justify-center gap-2 rounded-full bg-[#d6a252] px-6 sm:px-7 py-3 sm:py-4 text-xs sm:text-sm font-extrabold uppercase tracking-[0.08em] text-[#092522] shadow-[0_18px_36px_rgba(214,162,82,0.22)] transition-all hover:-translate-y-0.5 hover:bg-[#f0c975] active:scale-95"
            ...
            <button
              className="inline-flex min-h-[46px] sm:min-h-[52px] items-center justify-center gap-2 rounded-full border border-white/30 bg-white/10 px-6 sm:px-7 py-3 sm:py-4 text-xs sm:text-sm font-extrabold uppercase tracking-[0.08em] text-white backdrop-blur-md transition-all hover:bg-white/[0.18] active:scale-95"
        ...
        <motion.div
          className="mt-4 grid grid-cols-2 gap-2.5 md:hidden pb-4"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1], delay: 0.42 }}
        >
          <button
            className="rounded-[20px] border border-white/[0.16] bg-white/10 px-3.5 py-3 text-left backdrop-blur-md transition-all active:scale-95"
            onClick={() => onNavigate?.(QUICK_INFO_SLIDE)}
            type="button"
          >
            <span aria-hidden="true" className="material-symbols-outlined text-[22px]" style={{ color: '#f0c975' }}>restaurant</span>
            <span className="mt-1.5 block text-[10px] sm:text-[11px] font-extrabold uppercase tracking-[0.14em] text-white">{t('quickInfo.breakfast', lang)}</span>
          </button>
          <button
            className="rounded-[20px] border border-white/[0.16] bg-white/10 px-3.5 py-3 text-left backdrop-blur-md transition-all active:scale-95"
            onClick={() => onNavigate?.(SERVICES_SLIDE)}
            type="button"
          >
            <span aria-hidden="true" className="material-symbols-outlined text-[22px]" style={{ color: '#f0c975' }}>spa</span>
            <span className="mt-1.5 block text-[10px] sm:text-[11px] font-extrabold uppercase tracking-[0.14em] text-white">{t('services.poolTitle', lang)}</span>
          </button>
        </motion.div>
      </div>
    </section>
